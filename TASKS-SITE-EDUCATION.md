...
## Текущий приоритет (делаем сейчас)

**Цель:** сайт — основной интерфейс, мини-приложение в Telegram больше не развиваем.

| № | Что | Статус |
|---|-----|--------|
| 1 | **Загрузка графика** (.xlsx), парсинг, удобный интерфейс как в мини-приложении | ✅ Есть в Наряды: скачать шаблон, загрузить, просмотр по месяцам и датам, бригада на день |
| 2 | **Система опроса** (наряды + опрос девушек, кастомные) | ✅ Есть в Опросы: системные и групповые опросы |
| 3 | **Рейтинг** (очки, топ по курсу/институту, фильтры) | ✅ Есть: карточка на главной + полная страница с фильтрами |
| 4 | **Регистрация** при входе (Telegram ID, группа, год → сохранение в БД) | ✅ Сделано: POST /api/register, кнопка «Войти» регистрирует |
| 5 | **API без 500** (db.py fix, заглушки расписания, диагностика) | ✅ Сделано |

**Дальше по плану:** роли (teacher), форум без барахолки ✅, Novel, преподавательский кабинет — см. мастер-план ниже.

---

## Мастер-план: перенос на сайт (конспект)

### 1. Где мы сейчас

Сайт уже почти полностью перенёс функции мини-приложения. В `site/index.html` и `site/app.js` уже есть:

- **Наряды** (загрузка графика, ближайший наряд, шаблон)
- **Учёба** (расписание на неделю, место под журнал, пропуски, библиотеку, тренажёр)
- **Опросы** (системные + кастомные)
- **Планы** (задачник)
- **Рейтинг** (карточка + полная страница)
- **Профиль** (аватар, достижения, больничный, роли, админка)
- **Форум** (каркас разделов, барахолка убрана ✅)

Всё работает через единый API (`server.py`). Мини-приложение `app/` больше не развиваем — сайт главный.

---

### 2. Блок A. Сейчас (до Apex, только курсанты)

- Зафиксировать курсантский интерфейс: Наряды, Планы, Рейтинг, Профиль, Опросы — как есть.
- Форум: только служебные разделы (Общие, Учёба, Объявления, Галерея), без барахолки. Статус «в разработке».

---

### 3. Блок B. Роли и разделение прав

| Роль | Доступ |
|------|--------|
| Курсанты (user, sergeant, assistant) | Главная, Наряды, Учёба, Опросы, Планы, Рейтинг, Форум |
| Преподаватели (teacher) | Главная, модуль «Преподаватель», журнал/пропуски. **Нет доступа** к Нарядам, Опросам, Форуму |
| Админы | Всё + Админ-панель |

Реализовать: скрывать пункты меню по роли из `/api/profile/full`, проверка на бэкенде.

---

### 4. Apex ВУЗ — источник данных и авторизации

- Текущая форма входа (Telegram ID, группа, год) — fallback.
- После согласования: кнопка «Войти через Apex ВУЗ» → редирект → токен → ФИО, роль, группа → создание сессии.
- Расписание, пропуски, журнал — читать из их API, не дублировать.

---

### 5. Преподавательский кабинет

- Главная: только его расписание (группы, которые ведёт).
- Клик по паре → экран «Занятие»: тема, план (Novel), материалы, ДЗ, назначение тестов.
- Личная библиотека: курс → предмет → группа, список лекций и материалов.
- Конструктор тестов: техническая возможность создавать тесты, контент — преподаватели/кафедры.

---

### 6. Novel (React-редактор)

- Отдельный React-подпроект (Vite), `npm install novel`.
- Сборка dist/, раздача по `/novel/`, встраивание через `<iframe src="/novel/">` в раздел «Учёба» или конструктор тестов.
- Связка с API: сохранение/загрузка плана занятия.

---

### 7. Структура данных (для будущего)

- **Занятие:** тема, план (JSON/HTML), материалы, ДЗ, список тестов.
- **Тест:** вопросы, варианты, правильные ответы, привязка к группе/предмету.
...
10. **Форум:** внутреннее сообщество курсантов, анонимность по выбору, модерация (3+ жалобы → админу). См. **FORUM-ADMIN-POSTGRES.md**.
11. **PostgreSQL:** переход API сайта на PostgreSQL для надёжности; бот может остаться на SQLite или использовать общую БД. См. **FORUM-ADMIN-POSTGRES.md**.
