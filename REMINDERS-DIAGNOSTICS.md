...
**Где логика:**  
- Проверка каждые 30 секунд: `handlers/task_reminders.py` — `check_task_reminders()` (дедлайн в окне ±90 сек).  
- Точно в момент дедлайна: `restore_task_reminders()` при старте создаёт `job_queue.run_once(send_delayed_task_reminder, when=deadline)`.
...
### Шаг 3. Job при перезапуске

Как и для нарядов: при перезапуске бота все «разовые» job'ы теряются. `restore_task_reminders()` при старте заново создаёт job для каждой задачи с `reminded = 0` и будущим дедлайном.

**Проверка:** В логах при старте:
```text
Восстановлено N напоминаний
```
Если после установки напоминания бот перезапускали, убедитесь, что дедлайн был в будущем на момент старта.
...
| Бот не падал/не перезапускался | — | restore при старте создаёт job на будущие дедлайны |
